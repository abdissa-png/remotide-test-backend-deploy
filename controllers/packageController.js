...
const getPackages = catchAsync(async (req, res) => {
  ...
  const packagesWithDiscounts=packages.map((package) => {
    ...
    return {...package.toJSON(),discount:{...package.discount.toJSON(),discountedPrice:(originalPrice*((100-discountPercentage)/100))}};
  });
  res.status(200).json({
    status: 'success',
    data: {
...
const createPackage = catchAsync(async (req, res) => {
  ...
  const newDiscount = await Discount.create({...discountDetails,packageId:newPackage._id});
  const {discountPercentage,originalPrice} =newDiscount
  newPackage.discount=newDiscount._id;
  await newPackage.save();
  const packageWithDiscountDetails= {...newPackage.toJSON(),discount:{...newDiscount.toJSON(),discountedPrice:(originalPrice*((100-discountPercentage)/100))}}
...
const getPackageById = catchAsync(async (req, res) => {
  ...
  const package = await Package.findOne({_id:req.params.id,isActive:true}).populate("discount");
...
const updatePackageById = catchAsync(async (req, res) => {
  ...
  var discount;
  if (discountDetails) {
    discount = await Discount.create({...discountDetails,packageId:package._id})
  ...
  res.status(200).json({ status: 'success',message:"updated successfully", updatedPackage: {
    ...package.toJSON(),
    discount : {
      ...discount.toJSON(),
      discountedPrice:(originalPrice*((100-discountPercentage)/100))
